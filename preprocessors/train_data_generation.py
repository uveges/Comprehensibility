...
class Generator:
    ...
    def __init__(self,
        ...
        self.mode = mode
        if mode == 'bert':
            self.transformer_model = hu_core_news_trf.load()
        elif mode == 'splitter':
            self.sentence_splitter = SentenceSplitter(language='hu',
                                                      non_breaking_prefix_file=str(files("resources") / 'hu.txt'))
        ...
        for file in tqdm(os.listdir(self.txt_folder)):
            ...
            if filename.endswith(".txt"):
                ...
                if self.mode == 'splitter':
                    ...
                    try:
                        doc = self.transformer_model(content)
                        sentences = [sent.text.strip() for sent in doc.sents]
                        sentences = self.filter_out_noise_from_data(sentences, debug, filtered)
                    except RuntimeError as e:
                        print(e)
                        print(f"Unsegmented: {filename}")
                        unprocessable.append(filename)
                        continue
                else:
                    raise NotImplementedError('Please choose from ["bert", "splitter"]!')

                to_write = "\r\n".join(sentences)
                try:
                    file_to_write = os.path.join(self.sentence_segmented_folder, filename.replace(".txt",
                                                                                            config.TEMP_FILE_EXTENSIONS[
                                                                                                'sentence_segmented']))
